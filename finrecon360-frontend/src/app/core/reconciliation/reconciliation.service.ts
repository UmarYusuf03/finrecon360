...
export class ReconciliationService {
  ...
  getBankAccounts(): Observable<BankAccount[]> {
    return this.http.get<BankAccount[]>(`${this.apiUrl}/api/admin/bank-accounts`);
  }

  getAvailableImports(bankAccountId: string): Observable<BankStatementImport[]> {
    return this.http
      .get<PaginatedResponse<BankStatementImport>>(
        `${this.apiUrl}/api/admin/reconciliation/imports`,
        {
          params: {
            bankAccountId,
            pageNumber: 1,
  ...
  runAutomatedMatching(importId: string): Observable<MatchingSummaryResponse> {
    ...
    return this.http.post<MatchingSummaryResponse>(
      `${this.apiUrl}/api/admin/reconciliation/run-automated-matching`,
      payload,
    );
  }

  getProposedMatchGroups(): Observable<MatchGroup[]> {
    return this.http.get<MatchGroup[]>(`${this.apiUrl}/api/admin/reconciliation/proposed-match-groups`);
  }

  confirmMatches(matchGroupIds: string[]): Observable<ConfirmMatchesResponse> {
    return this.http.post<ConfirmMatchesResponse>(
      `${this.apiUrl}/api/admin/reconciliation/confirm-matches`,
      { matchGroupIds },
    );
  }

  getExceptions(): Observable<BankStatementLine[]> {
    return this.http.get<BankStatementLine[]>(`${this.apiUrl}/api/admin/reconciliation/exceptions`);
  }
}
