...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatCardModule, MatListModule, MatCheckboxModule, MatButtonModule, MatIconModule, MatSnackBarModule, RouterLink],
...
export class ReconciliationReviewComponent implements OnInit {
  ...
  private loadProposedGroups(): void {
    ...
    this.reconciliationService
      ...
      .subscribe({
        ...
        error: () => this.snackBar.open('Unable to load proposed matches.', 'Close', { duration: 3000 }),
      });
  }
  ...
  confirmSelected(): void {
    ...
    this.reconciliationService.confirmMatches(ids).subscribe({
      ...
      error: () => this.snackBar.open('Confirmation failed. Please try again.', 'Close', { duration: 3000 }),
    });
  }
}
